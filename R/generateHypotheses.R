...
generateBinaryNetwork <- function(complex_hypothesis){
  ...
  for(i in c(1:length(complexes))){
    binary_interactions <- get_interactions(complex=complexes[i],hypothesis=complex_hypothesis,binary_interactions=binary_interactions)
  }
...
#' Append binary interaction table with additional complexes
#' @import data.table
...
#' @export
get_interactions <- function(complex,hypothesis,binary_interactions){
