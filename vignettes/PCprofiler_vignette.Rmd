...
## Small sample complex sets as used in previous 
## examples are too small for decoy generation.
...
proteinFeaturesFiltered <- filterGridSearchResults(proteinFeatures_grid,
...
proteinFeaturesFilteredStats <- estimateGridSearchDecoyFDR(proteinFeaturesFiltered)

proteinFeatures_bestParameterStats <- getBestParameterStats(proteinFeaturesFilteredStats,
                                                            FDR=0.1)
proteinFeatures_bestParameterData <- getBestParameterData(proteinFeaturesFiltered,
                                                          FDR=0.1)
plotIdFDRspace(proteinFeaturesFilteredStats,
               level = "complex",
               id_level = "TP",
               FDR_cutoff = 0.1,
...
complexFeaturesFiltered <- filterGridSearchResults(complexFeatures_grid,
                                                        peak_corr_cutoffs = c(0.5,0.75,0.9),
                                                        feature_completeness_cutoffs = c(0,0.5,1),
                                                        n_subunits_cutoffs =c(2,3,4),
                                                        remove_decoys=FALSE
...
# test if FDR is done on bestFeature only
complexFeaturesFilteredStats <- estimateGridSearchDecoyFDR(complexFeaturesFiltered)

complexFeatures_bestParameterStats <- getBestParameterStats(complexFeaturesFilteredStats,
                                                            FDR=0.1)
complexFeatures_bestParameterData <- getBestParameterData(complexFeaturesFiltered,
                                                          FDR=0.1)

plotIdFDRspace(complexFeaturesFilteredStats,
               level = "complex",
               id_level = "TP",
               FDR_cutoff = 0.1,
               colour_parameter = "min_feature_completeness")
